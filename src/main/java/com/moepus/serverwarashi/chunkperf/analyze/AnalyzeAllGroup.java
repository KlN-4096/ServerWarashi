...
import com.moepus.serverwarashi.chunkperf.data.analyze.AllGroupSession;
 ...
 * 全分组性能会话跟踪器。
 */
public final class AnalyzeAllGroup extends AbstractGroupAnalyzer<AllGroupSession> {
    private final ChunkPerfGroupIndex groupIndex;

    public AnalyzeAllGroup(ChunkPerfGroupView groupView, ChunkPerfGroupIndex groupIndex) {
        super(groupView);
    ...
    private StartResult<AllGroupSession> prepareStart(ServerLevel level,
        ...
        List<ChunkPerfSnapshot.ChunkPerfGroupEntry> groups =
                groupView.groups(level, ChunkPerfSnapshot.PauseMode.ACTIVE_ONLY);
        if (groups.isEmpty()) {
            return startError(ChunkPerfMessages.noTicketGroupsFound());
        }
        ...
        return startSuccess(
                session,
                ChunkPerfMessages.allGroupsAnalysisStarted(level.dimension(), groups.size(), effectiveDuration)
        );
    }
    ...
    protected Component buildReport(AllGroupSession session) {
        ...
        for (int i = 0; i < session.groups.size(); i++) {
            ChunkPerfSnapshot.ChunkPerfGroupEntry entry = session.groups.get(i);
